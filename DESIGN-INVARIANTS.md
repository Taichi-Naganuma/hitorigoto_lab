...
- **1-2 アクセントは Operator Blue 1色（多色化しない）**: アクセントは **Operator Blue 1色 `#2A50DA`**（深コバルト）
  に固定。旧 `#0095f6`（SNS青）と **シアン→紫グラデ（`#22d3ee`→`#8b5cf6` ＝ AIハイプのクリシェ）は棄却**。
  生気は**アクセント青の極薄境界**で担保し、常時多色に散らさない。操作/リンク/ブランドは青に集約する。
...
- **1-3b 証跡は「金」で前傾（希少）**: 証拠の前傾は **証跡ゴールド `#C6890B`** で担保。面積<5%、ボタン・境界・
  見出しには使わない（金＝非アラート証跡／橙 `warning` ＝アラート、で用途分離）。
...
| トークン | 確定値 | 備考 |
|---|---|---|
| アクセント | Operator Blue `#2A50DA`（`--accent`） | 1色固定（§1-2・2026-07-31 改訂） |
| 証跡ゴールド | `#C6890B`（`--gold`） | ライブ数値/証跡のみ・面積<5%（§1-3b） |
| セマンティック | success `#12805C`／warning `#DC6803`／danger `#D0392B` | 機能色 |
| ホバー境界線 | アクセント青の極薄境界（旧 cyan→紫は撤去） | 常時多色化しない（§1-2） |
| 背景（顔） | 全面ライト（オフ白系） | §1-1 |
